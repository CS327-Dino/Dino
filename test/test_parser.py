...
class TestParser(unittest.TestCase):
    ...
    def test_list_methods(self):
        ...
        self.assertEqual(expression, Seq(things=[Echo(expr=BinOp(left=Identifier(name='a', line=1, isconst=False, uid=4), op=TokenType.DOT, right=MethodLiteral(name='length', args=[], line=1), line=1), line=1)]))

        expression = Parser(Scanner("echo(a.head);", DinoError()).generate_tokens(), DinoError()).parse()
        self.assertEqual(expression, Seq(things=[Echo(expr=BinOp(left=Identifier(name='a', line=1, isconst=False, uid=6), op=TokenType.DOT, right=MethodLiteral(name='head', args=[], line=1), line=1), line=1)]))

        expression = Parser(Scanner("echo(a.tail);", DinoError()).generate_tokens(), DinoError()).parse()
        self.assertEqual(expression, Seq(things=[Echo(expr=BinOp(left=Identifier(name='a', line=1, isconst=False, uid=8), op=TokenType.DOT, right=MethodLiteral(name='tail', args=[], line=1), line=1), line=1)]) )

        expression = Parser(Scanner("echo(a.slice(0,3));", DinoError()).generate_tokens(), DinoError()).parse()
        self.assertEqual(expression, Seq(things=[Echo(expr=BinOp(left=Identifier(name='a', line=1, isconst=False, uid=10), op=TokenType.DOT, right=MethodLiteral(name='slice', args=[IntLiteral(value=0, line=1), IntLiteral(value=3, line=1)], line=1), line=1), line=1)]))
